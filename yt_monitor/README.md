...
유행 쇼츠를 모은 뒤 **숫자 · 제목 · 시간 · 첫 화면**을 분석해서 영상 만들기에 자동으로 반영합니다 (🔥 트렌드 탭 맨 위에 리포트).

| 분석 | 반영되는 곳 |
|---|---|
| ⏱ 길이: 시간당 조회수 상위 영상의 길이 중앙값 | 대본 목표 길이 (20~58초 사이, `trends.auto_optimize`) |
| ✍ 제목: 평균 글자 수, 숫자 · 질문 · 대문자 강조 · POV · 1인칭 비율과 "있을 때 시간당 조회수 몇 배" | 제목 후보 생성 |
| 👍 반응: 좋아요율 · 댓글률, 댓글이 폭발한 영상 | 리포트 |
| 🕒 올린 시간: 미국 동부 기준 3시간 단위로 잘 뜬 시간 → 한국 시간으로 | 업로드 정보 창의 "추천 업로드 시간" |
| 🖼 썸네일(첫 화면): 큰 글씨 · 얼굴 · 클로즈업 · 표정 · 훅 (비전 모델) | 대본 주제 · 첫 장면 AI 이미지 스타일 |

- 썸네일 분석은 Ollama 비전 모델이 필요합니다: 명령 프롬프트에서 `ollama pull qwen2.5vl:7b` (약 6GB, 한 번만).
  없으면 이 부분만 건너뜁니다. 분석 뒤에는 모델을 그래픽카드에서 내립니다.
- **캐시(효율)**: 수집 결과는 `data/trend_cache.json`에 저장돼서, 3시간 안에 원클릭을 다시 누르면 **YouTube 쿼터 0 · 바로 시작**합니다.
  썸네일 분석 결과도 영상별로 저장해서 같은 영상은 다시 분석하지 않습니다. [🔥 유행 쇼츠 분석] 버튼은 항상 새로 모읍니다.
- 주제를 직접 적은 원클릭도 최근 분석 결과(길이 · 제목 패턴 · 업로드 시간 · 첫 화면)를 반영합니다.
- 표본이 "이미 뜬 영상"뿐이라 **경향을 보는 참고용**입니다.

## 📈 내 채널 학습 (올린 영상 성과 → 다음 영상에 반영)

내가 올린 쇼츠 중 **무엇이 잘 됐는지** 보고 다음 영상을 그쪽으로 맞춥니다. 모델을 새로 학습시키는 딥러닝이 아니라,
내 채널 데이터로 다음 선택을 바꾸는 **피드백 방식**입니다.

1. **성과**: 영상마다 게시 후 **48시간 조회수**(수집 기록을 보간)를 채널 중앙값과 비교 → "평소의 몇 배"
2. **연결**: 이 프로그램이 만든 영상 기록(제목 후보 · 분위기 · 음성 · 길이 · 배경음악)을 실제로 올라간 영상과 **제목으로 자동 연결**
3. **비교**: 잘 된/안 된 영상 제목, 길이, 제목 패턴(숫자 · 질문 · 대문자 …), 올린 시간, 분위기, 음성
4. **반영**: 주제 · 대본 프롬프트("이 채널에서 잘 된 것/안 된 것"), 제목 후보, 대본 길이(유행 분석 값과 반반), 추천 업로드 시간(내 채널 기준 먼저)

- 48시간 지난 내 쇼츠가 **6개 이상** 쌓이면 켜집니다 (`learning.min_videos`). 그 전에는 트렌드 탭에 "데이터 모으는 중 (n/6)"만 보입니다.
- 원클릭을 누를 때 내 채널 조회수가 6시간 넘게 오래됐으면 자동으로 새로 가져옵니다 (쿼터 몇 unit).
  처음 한 번은 **[▶ 지금 체크하기]**로 내 채널을 수집해 두세요. **자동 체크**를 켜 두면 기록이 더 촘촘해집니다.
- 제목을 업로드 정보 창의 후보 중에서 고르거나 조금만 고치면 자동으로 연결됩니다 (완전히 다른 제목이면 연결 안 됨).
