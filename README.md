...
관습형(C) 6개 유형을 유형별 10문항씩(흥미·자신감·행동 습관·가치관 등
서로 다른 측면을 물어 점수가 몇 값으로 뭉치지 않도록 함) 총 60문항(1~5점
리커트 척도)으로 측정하고, 점수가 가장 높은 두 유형을 조합한 홀랜드
코드를 계산해 이에 가까운 전공 30여 개를 적합도 순으로 추천합니다. 각
문항은 특정 유형이 연달아 드러나지 않도록 유형을 섞어서 배치했습니다.

- `/aptitude` — 검사 응시 및 결과(유형별 점수 막대그래프, 추천 전공/관련
  직업)를 보여주는 웹 페이지
- `/api/aptitude` — `{"answers": {"R1": 5, "I1": 3, ...}}` 형태로 60개
...
- `aptitude/questions.py` — 유형별 10문항씩 총 60개 검사 문항
