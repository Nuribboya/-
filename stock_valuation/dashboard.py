...
with st.expander("대형주 + 매출 안정형 포트폴리오 (실제 매매 전 반드시 직접 검토하세요)"):
    st.caption(
        ...
        "부채비율도 안정적이고 영업이익률도 건전한 종목만 골라서 시가총액이 큰 순서로 상위 N개를 "
        "시총 비중으로 배분합니다. 스캔할 때 '연간 매출 일관성 체크' 옵션을 켜야 이 섹션이 동작해요. "
        "`entry_zone`은 현재가가 그 종목 자신의 52주 저점/이동평균선 대비 어디쯤 있는지 "
        "보여주는 참고용 지표입니다 (미래를 예측하는 게 아니라 지금 위치를 설명하는 것). "
        "`rally_support`는 같은 기간 동안 주가 상승폭을 매출 성장폭과 비교해서, 그 상승이 "
        "매출 성장으로 뒷받침되는지 아니면 주가만 훨씬 빠르게 오른 버블성인지 참고용으로 보여줍니다."
    )
    ...
    with scol6:
        require_expense_efficiency = st.checkbox("영업이익률 건전한 종목만", value=True)
    ...
    if require_expense_efficiency and "expense_efficiency_ok" not in df:
        st.warning(
            "이 결과 파일엔 expense_efficiency_ok 컬럼이 없어서 지출 효율 필터를 못 써요 — 다시 스캔해보세요."
        )
        require_expense_efficiency = False
    ...
    if "revenue_consistency_ok" not in df or "market_cap" not in df:
        ...
        steady_portfolio = build_steady_growth_portfolio(
            ...
            require_expense_efficiency=require_expense_efficiency,
        )
