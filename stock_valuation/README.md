...
- **상승이 버블성인지 (`rally_support`, `rally_support_reason`)**: 가격이
  올랐다는 사실 자체만으로는 그게 실적이 좋아져서 오른 건지, 아니면
  실적과 무관하게 가격만 먼저 튄 건지 알 수 없습니다. 그래서 연간 매출
  일관성 체크와 같은 기간(보유한 연간 매출 히스토리 시작~끝 연도)에 대해
  "그 기간 동안 주가는 몇 % 오르고 매출은 몇 % 늘었는지"를 비교합니다 —
  둘 다 이미 갖고 있는 데이터라 새로 조회하지 않습니다. 주가 상승폭이
  매출 성장폭보다 3배 이상 빠르면 "버블성 상승 우려", 그 정도로 벌어지지
  않았으면 "펀더멘털 뒷받침된 상승"으로 표시됩니다
  (`entry_timing.classify_rally_support`). `entry_zone`처럼 필터가
  아니라 참고용 정보 컬럼이라 포트폴리오 선정 자체에는 영향을 주지
  않고, 연간 매출 일관성 체크를 켰을 때만 계산됩니다.

> ⚠️ 연간 재무제표 데이터가 보통 4개년치뿐이라 "몇 년"의 기준이 넉넉하진
> 않습니다. 또 "매출이 꾸준했다"는 과거 얘기지 미래를 보장하지 않고요.
> 여기도 위 포트폴리오와 마찬가지로 종목/섹터 상관관계는 전혀 안 보는
> 단순 분산 규칙입니다. `entry_zone`/`rally_support`도 마찬가지로 "가격이
> 어디 있는지", "상승 배경이 뭐로 보이는지"를 설명하는 것뿐, "여기서 사면
> 오른다"는 예측이 아닙니다 — 특히 `rally_support`는 4개년치 정도의
> 얕은 연간 데이터로 "버블이냐 아니냐"를 가르는 거라 판정이 거칠 수밖에
> 없다는 점을 감안하고 참고용으로만 쓰세요.
